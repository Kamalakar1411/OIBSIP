...
text_widget_font = ("Times New Roman", 12)
window = Tk()
window.title("BMI Calculator")
eh_label = Label(window, text="Enter your height in meters",font=text_widget_font)
eh_label.grid(row=0, column=0)
eh_value = StringVar()
eh = Entry(window, textvariable=eh_value)
eh.grid(row=1, column=0)

ew_label = Label(window, text="Enter your weight in kg",font=text_widget_font)
ew_label.grid(row=2, column=0)
ew_value = StringVar()
ew = Entry(window, textvariable=ew_value)
ew.grid(row=3, column=0)

resultdisplay = Text(window, height=10, width=50,font=text_widget_font)
resultdisplay.grid(row=0, column=1, rowspan=4)

calc = Button(window, text="Calculate", height=3, width=30, command=bmi_calc,font=text_widget_font)
